...
Recenlty when using conda to create an environment, I used a script that was written about three months ago. At that time, I ran the code to install the python dependencies and it worked fine on my [Kaggle](https://www.kaggle.com) notbook. But when I tried to run it again yesterday, it reported error after searching a specific package ( in this case it's Cython version mismatch when installing a version of murmurhash). 
