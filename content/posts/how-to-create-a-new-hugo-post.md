...
After a hugo blog is set up on a local computer, to create and publish a new blog, you need to use the `hugo new` command. It generates a new markdown file starting with its creationg date, draft status and title. To run this command, first go to the site directory that contains all of the blog files within, e.g., one of them is the *content* folder that we access below.
...
Finally, using git command to commit and push the blog file to remote server.
